...
class TurbofanDatamodule(pl.LightningDataModule):
    def __init__(self, batch_size=64, random_state=10):
        super().__init__()

        self.batch_size = batch_size
        self.g_cpu = torch.Generator()
        self.g_cpu.manual_seed(random_state)
    ...
    def train_dataloader(self):
        return DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=True, generator=self.g_cpu)

    def val_dataloader(self):
        return DataLoader(self.val_dataset, batch_size=self.batch_size, generator=self.g_cpu)

    def test_dataloader(self):
        return DataLoader(self.test_dataset, batch_size=self.batch_size, generator=self.g_cpu)

    def predict_dataloader(self):
        return DataLoader(self.predict_dataset, batch_size=self.batch_size, generator=self.g_cpu)
