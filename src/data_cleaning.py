...
def identify_missing_values(df: pd.DataFrame, threshold: float = 0.1, drop: bool = False) -> Union[pd.DataFrame, List[str]]:
    ...
    :return: Cleaned data or list of features with missing values above the threshold.
...
def identify_single_unique_features(df: pd.DataFrame, drop: bool = False) -> Union[pd.DataFrame, List[str]]:
    ...
    :return: Cleaned data or list of features with only a single unique value.
    :rtype: pd.DataFrame or list
    """
...
def get_uncorrelated_features(df: pd.DataFrame, threshold: float = 0.2, target: str = None) -> List[str]:
    ...
    :param threshold: The correlation threshold to use. Default is 0.9.
                      Features with an absolute correlation coefficient less than this value with the target are
...
def clean_data(train_df: pd.DataFrame, test_df: pd.DataFrame, method: Union[str, None] = 'winsorize', ignore_columns: List[str] = None, contamination: float = 0.05, threshold_sd: float = 0.8, threshold_missing: float = 0.1, soft_drop: bool = False, threshold_corr: float = 0.9) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    logger.info("Filter and drop features based on missing values, single unique values, and target correlation...")
    ...
    single_unique_features = identify_single_unique_features(train_df)

    # get uncorrelated features with the target
    train_df_with_target = calculate_RUL(train_df, time_column='Cycle', group_column='UnitNumber')
    uncorrelated_features = get_uncorrelated_features(train_df_with_target, threshold=threshold_corr, target='RUL')
    ...
    cleaned_train_df = train_df.drop(columns=features_to_drop)
    cleaned_test_df = test_df.drop(columns=features_to_drop)
    ...
    cleaned_train_df = remove_outliers(cleaned_train_df, method=method, ignore_columns=ignore_columns, contamination=contamination, threshold_sd=threshold_sd, soft_drop=soft_drop)
    cleaned_test_df = remove_outliers(cleaned_test_df, method=method, ignore_columns=ignore_columns, contamination=contamination, threshold_sd=threshold_sd, soft_drop=soft_drop)

    # impute missing values (using spline interpolation)
    if cleaned_train_df.isnull().sum().sum() > 0 or cleaned_test_df.isnull().sum().sum() > 0:
        logger.info(f"Imputing {cleaned_train_df.isnull().sum().sum()} missing values in the train DataFrame and {cleaned_test_df.isnull().sum().sum()} missing values in the test DataFrame...")
        cleaned_train_df = cleaned_train_df.interpolate(method='spline', order=3)
        cleaned_test_df = cleaned_test_df.interpolate(method='spline', order=3)
