...
def plot_sensor_measures_from_dataset(config, data, name_of_dataset, show=True, save=False):
    ...
    sensor_measure_columns_names = list(filter(lambda x: x.startswith('Sensor'), data.keys()))
    number_of_columns = len(sensor_measure_columns_names)
    fig = plt.figure(figsize=(15,2.5*number_of_columns))
    for i,v in enumerate(range(number_of_columns)):
        a = plt.subplot(number_of_columns,1,v+1)
    ...
    if save:
        timestamp = time.strftime("%Y%m%d-%H%M%S")
        fig.savefig(f"{config['paths']['plot_dir']}ex2_sensor_measures_dataset_{name_of_dataset}_{timestamp}.png")
    plt.close()


def plot_single_sensor_measure_from_dataset(config, data, sensor_name, name_of_dataset, show=True, save=False):
    """ This method plots the sensor measures from all motors from the given dataset. Furthermore, it plots the mean
    signal of the sensor measures in red.
    ...
    a = plt.subplot()
    min_rul = data.groupby("UnitNumber")["RUL"].max().min()
    a.plot(range(min_rul), data[data["RUL"] <= min_rul].groupby("RUL")[sensor_name].
           mean(), color='r')
    for motor_id in data['UnitNumber'].unique():
        a.plot(range(min_rul-1, -1, -1), data[data["UnitNumber"] == motor_id][sensor_name][-min_rul:],
               alpha=0.1, color='b')
    a.invert_xaxis()
    a.title.set_text(name_of_dataset + ': ' + sensor_name)
    plt.tight_layout()
    if show:
        plt.show()
    if save:
        timestamp = time.strftime("%Y%m%d-%H%M%S")
        fig.savefig(f"{config['paths']['plot_dir']}ex2_sensor_measure_from_{sensor_name}_dataset_{name_of_dataset}"
                    f"_{timestamp}.png")
    plt.close()
